...
// UnimplementedChaincodeSupportServer must be embedded to have
...
func RegisterChaincodeSupportServer(s grpc.ServiceRegistrar, srv ChaincodeSupportServer) {
	// If the following call panics, it indicates UnimplementedChaincodeSupportServer was
...
// UnimplementedChaincodeServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
func RegisterChaincodeServer(s grpc.ServiceRegistrar, srv ChaincodeServer) {
	// If the following call panics, it indicates UnimplementedChaincodeServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
